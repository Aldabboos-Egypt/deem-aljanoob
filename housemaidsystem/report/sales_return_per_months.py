...
class SalesReturnPerMonths(models.Model):

    _name = "housemaidsystem.report.salesreturnpermonths"
    _auto = False
    _description = "Sales Return Per Months"
    _rec_name = 'salse_man'
    ...
    def _from(self):
        return """
            From
            account_invoice , housemaid_applicant_applications , housemaid_applicant_selltest, res_partner, res_users
        """

    def _where(self):
        return """
            WHERE
              housemaid_applicant_applications.id = housemaid_applicant_selltest.application_id AND
              housemaid_applicant_selltest.new_invoice_id = account_invoice.id AND
              housemaid_applicant_selltest.sales_man = res_users.id AND
