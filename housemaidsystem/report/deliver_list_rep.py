...
class ReservationsListRep(models.AbstractModel):
    _name = "report.housemaidsystem.deliver_list_rep"
    _description = "Deliver List Report"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data['from_date']:
            invoice_domain.append(
                ('deliver_date', '>=', data['from_date']),
            )
            rep_head_line1 = 'From Date: ' + data['from_date']
        # 2) To Date
        if data['to_date']:
            invoice_domain.append(
                ('deliver_date', '<=', data['to_date']),
            )
        ...
        if data['invoice_status']:
            invoice_domain.append(
                ('invoice_id.state', '=', data['invoice_status'])
            )
            rep_head_line2 = 'Invoice Status: ' + data['invoice_status']

        docs = self.env['housemaidsystem.applicant.deliver'].search(invoice_domain)



        docargs = {
            'doc_ids': docids,
            'doc_model': 'housemaidsystem.applicant.deliver',
            'docs': docs,
